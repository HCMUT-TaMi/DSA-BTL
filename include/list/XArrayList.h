...
void XArrayList<T>::add(int index, T e)
{
    ...
    //  Check if index is valid and Check if need to update the size 
    this->checkIndex(index); 
    this->ensureCapacity(this->count); 


    //  Run back_ward [1,2,3,4] -> add(2,5) -> [1,2,2,3,4] -> insert 5 at 2 
...
T XArrayList<T>::removeAt(int index)
{
    ...
    if(index < 0 || index >= count) throw std::out_of_range("out_of_range");

    //  Hold on for the position we need!
    T del = this->data[index]; 

    //  run for_ward [1,2,3,4] -> remove(2) -> [1,2,4,4] -> this->count-- -> [1,2,4]
    ...
    //  User have to ensure their own of safety 
    return del; 
}
...
{
    // TODO
    XArrayList<T>::Iterator it; 

    // Finding the possition 
    ...
    { 
        ...
        {
            if(itemEqual(*it,item)) break; 
        }
    }

    if(!(it != end())) return false; 

    // Use the delete used by our friend 
    it.remove(); 

    //  Count update through iter::remove -> Array::removeAt()
    return true;
}
...
string XArrayList<T>::toString(string (*item2str)(T &))
{
    ...
    //  intial setup
    XArrayList<T>::Iterator it = this->begin(); 
...
void XArrayList<T>::ensureCapacity(int index)
{
    ...
    //  Ensure that the index bounds between Count_index
    checkIndex(index); 

    //  Check if over_size is occurs! 
    if(index == this->capacity)
    {

        try
        { 
            T *dum = new T[capacity*2];
            for(int i = 0; i < this->count; i++) dum[i] = this->data[i];
            this->capacity *= 2; 
            delete[] data; 
            this->data = dum;  
        }

        //  In case that the allocation is not usable
