...
public:
    ...
    virtual ~AbstractGraph(){this->nodeList.setDeleteUserDataPtr(&DLinkedList<VertexNode*>::free);}
...
//////////////////////////////////////////////////////////////////////
    ...
    virtual bool contains(T vertex){
        //TODO
        VertexNode* pos = this->getVertexNode(vertex);
        return pos; 
    }

    virtual float weight(T from, T to){
        //TODO
        VertexNode* u = this->getVertexNode(from);
        VertexNode* v = this->getVertexNode(to); 
        if(!u) throw VertexNotFoundException(vertex2str(from)); 
        if(!v) throw VertexNotFoundException(vertex2str(to)); 

        Edge* e = u->getEdge(v); 
        if(!e) 
        {
            Edge findingEdge(u, v);
            throw EdgeNotFoundException(edge2Str(findingEdge));
        }
    ...
    virtual DLinkedList<T> getOutwardEdges(T from){
        ...
        return u->getOutwardEdges(); 
    }
    
    virtual DLinkedList<T>  getInwardEdges(T to){
        //TODO
        DLinkedList<T> getList; 
        VertexNode* v = this->getVertexNode(to); 
        if (!v)
        ...
        while(it != nodeList.end()){
            VertexNode* u = *it;
                ...
                //  Check if there is connection using VertexNode::getEdge 
                Edge* tr = u->getEdge(v); 
                if(tr != nullptr)
                {
                    getList.add(u->getVertex()); 
                }
    ...
    virtual int inDegree(T vertex){
        //TODO
        VertexNode* u = this->getVertexNode(vertex); 
        if(!u) throw VertexNotFoundException(vertex2str(vertex)); 
        return u->inDegree(); 
    ...
    virtual bool connected(T from, T to){
        //TODO
        VertexNode* uVer = this->getVertexNode(from); 
        VertexNode* vVer = this->getVertexNode(to); 
        ...
        return (uVer->getEdge(vVer) != nullptr); 
    }
...
//BEGIN of VertexNode    
    ...
    public:
        ...
        DLinkedList<T> getOutwardEdges(){
            ...
            for(; it != this->adList.end(); it++)
            {
                Edge* atEdge = *it;
                GET.add(atEdge->to->vertex);
            }
        ...
        bool equals(VertexNode* node){
            //TODO
            return vertexEQ(this->getVertex(),node->getVertex());
        }
        
        void removeTo(VertexNode* to){
            //TODO
            Edge* delEdge = this->getEdge(to);

            if(delEdge)
            {
                bool isDel = this->adList.removeItem(delEdge,[](Edge* V){delete V;}); 
                this->outDegree_--; 
                to->inDegree_--; 
            }
