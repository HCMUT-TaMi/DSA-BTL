...
xMap<K,V>::xMap(
    ...
    //  Ensure that table is intial, NEED TO DELETE [1]
...
V xMap<K,V>::put(K key, V value){
    ...
    V retValue = value;
    //YOUR CODE IS HERE   
    //  COST O(N) time to run in avarage and O(1) for best case
    DLinkedList<Entry*>& travel = table[index];
    
    // Travel through all of the nodes by Iterators 
    for(auto find : travel)
    {
        if(keyEQ(find->key,key))
        {
            retValue = find->value; 
            find->value = value; 
            return retValue; 
    ...
    //  When we not find Keys 

    Entry* new_entry = new Entry(key,value); 
    
    //  Insert at the end
    travel.add(new_entry); 
...
V xMap<K,V>::remove(K key,void (*deleteKeyInMap)(K)){
    ...
    //  Travel through all of the array
    for(typename DLinkedList<Entry*>::Iterator it = travel.begin(); it != travel.end(); it++)
    {
        Entry *& travel = *it; 
        if(keyEQ(travel->key,key)) 
        {
            V retValue = travel->value; 

            if(deleteKeyInMap) 
            {
                deleteKeyInMap(travel->key);
            }

            it.remove(&deleteEntry); 
...
bool xMap<K,V>::remove(K key, V value, void (*deleteKeyInMap)(K), void (*deleteValueInMap)(V))
{
    ...
    DLinkedList<Entry*>& travel = this->table[index]; 

    for(typename DLinkedList<Entry*>::Iterator it = travel.begin(); it != travel.end(); it++)
    {
        Entry *& travel = *it; 
        if(keyEQ(travel->key,key) && valueEQ(travel->value,value))  
        {
...
bool xMap<K,V>::containsKey(K key){
    ...
    //  Using Entry* to find out the pos we need 
    for(auto ptr : travel)
    {
        if(keyEQ(ptr->key,key)) return true; 
    }
...
bool xMap<K,V>::containsValue(V value){
    ...
    { 
        DLinkedList<Entry*>& travel = this->table[i]; 
    
        for(auto ptr : travel)
        {
            if(valueEQ(ptr->value,value)) return true; 
        }
    }
...
DLinkedList<K> xMap<K,V>::keys(){
    //YOUR CODE IS HERE 
    DLinkedList<K> getList; 

    for(int i = 0; i < capacity; i++)
    {
        DLinkedList<Entry *>& travel = table[i]; 

        for(Entry* val:travel)
        {
            getList.add(val->key); 
        }
    }
...
DLinkedList<int> xMap<K,V>::clashes(){
    //YOUR CODE IS HERE 
     DLinkedList<int> getList; 

    for(int i = 0; i < capacity; i++)
    {
        DLinkedList<Entry *>& travel = table[i];
        getList.add(travel.size()); 
    }
